...
func checkDependencies(c spi.Component, context spi.ComponentContext, visited map[string]bool, stateMap map[string]bool) (map[string]bool, error) {
	...
	for _, dependencyName := range c.GetDependencies() {
		...
		if !isInReadyState(context, dependency) && // CR status does not already indicate ready status
			dependency.IsEnabled(context.EffectiveCR()) && !dependency.IsReady(context) {
...
func isInReadyState(context spi.ComponentContext, comp spi.Component) bool {
	dependencyStatus, ok := context.ActualCR().Status.Components[comp.Name()]
	if ok {
		context.Log().Infof("Dependency %s state: %s", comp.Name(), dependencyStatus.State)
		// We've already reported Ready status for this component
		if dependencyStatus.State == vzapi.CompStateReady {
			context.Log().Infof("Dependency %s in Ready state", comp.Name())
			return true
		}
	}
	return false
