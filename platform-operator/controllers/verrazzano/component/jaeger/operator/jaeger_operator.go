...
func generateOverridesFile(ctx spi.ComponentContext, contents []byte) (string, error) {
	...
	ctx.Log().Debugf("Verrazzano jaeger-operator install overrides file %s contents: %s", overridesFileName, string(contents))
